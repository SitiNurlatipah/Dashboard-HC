...
class MppRealController extends Controller
{
    public function index(){
        ...
        $jobsupplyMpp = MppModel::select(DB::raw("CAST(SUM(mppJobsupply)as int) as jobsupplyMpp"))
					...
					->orderBy('tahun', 'asc')
                    ->pluck('jobsupplyMpp');
        $totalMpp = RealModel::join('mpp_employee','mpp_employee.idmpp','=','real_employees.mpp_tahun')
    ...
    public function updateMpp(Request $request, $idmpp){
        $mpp = MppModel::find($idmpp);
        $mpp->mppTotal = $request->mppTotal;
        $mpp->mppPermanent = $request->mppPermanent;
        $mpp->mppContract = $request->mppContract;
        $mpp->mppJobsupply = $request->mppJobsupply;
        $mpp->tahun = $request->tahun;
        $mpp->save();
        return redirect()->route('mppreal')->with('message','Data updated successfully.');
	}
    public function destroyMpp($idmpp)
    {
        MppModel::find($idmpp)->delete();
        // $data->delete();
        return redirect()->route('mppreal')->with('message','Data deleted successfully');
    }
    ...
    $real = RealModel::join('mpp_employee','mpp_employee.idmpp','=','real_employees.mpp_tahun')
    ->orderBy('dateBulan', 'desc')
    ->whereYear('dateBulan',$request->year)
    ->get();
    $mpp_employee=MppModel::all();       
